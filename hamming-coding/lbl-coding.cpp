...
// НОВАЯ ФУНКЦИЯ: сохранение закодированного файла (последовательность битов '0' и '1')
void saveEncodedFile(const string &inputFilename, const string &outputFilename,
                     const map<char, string> &codes)
{
    ifstream inFile(inputFilename, ios::binary);
    if (!inFile.is_open())
    {
        cerr << "Не удалось открыть входной файл " << inputFilename << " для сохранения кода." << endl;
        return;
    }
    ...
    if (!outFile.is_open())
    {
        cerr << "Не удалось создать выходной файл " << outputFilename << endl;
        inFile.close();
        return;
    }
    ...
    while (inFile.get(ch))
    {
        ...
        if (it != codes.end())
        {
            outFile << it->second; // записываем битовую строку как символы '0' и '1'
        }
        else
        {
            cerr << "Предупреждение: символ '" << ch << "' не имеет кода. Пропущен." << endl;
        }
    }

    inFile.close();
    outFile.close();
    cout << "Закодированный файл сохранён: " << outputFilename << endl;
}

int main()
{
    // Список файлов для обработки
    vector<string> filenames = {"./lbl-files/file1.txt", "./lbl-files/file2.txt", "./lbl-files/file3.txt"};
    ...
    for (const string &fname : filenames)
    {
        ...
        for (auto &method : methods)
        {
            ...
            // ---------- ДОБАВЛЕННО: сохранение закодированного файла ----------
            // Формируем имя выходного файла: исходное имя + "_" + метод + ".enc"
            string outFilename = fname + "_" + methodName + ".enc";
            saveEncodedFile(fname, outFilename, codes);
        }
        cout << endl;
    }
